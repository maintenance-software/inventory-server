                ...
                entityId <- if workOrderId > 0 then
                                do
                                  ...
                                  _ <- runDB $ update workOrderKey [ WorkOrder_EstimateDuration =. estimateDuration
                                                                   ...
                                                                   , WorkOrder_GeneratedById =. case generatedById of Nothing -> Nothing; Just a -> Just ((toSqlKey $ fromIntegral $ a)::Person_Id)
                                                                   , WorkOrder_ResponsibleId =. case responsibleId of Nothing -> Nothing; Just a -> Just ((toSqlKey $ fromIntegral $ a)::Person_Id)
